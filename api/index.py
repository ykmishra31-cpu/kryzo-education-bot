...
class PersistentSessions(dict):
    """Small dict-like session cache backed by Upstash so multi-instance Vercel
    invocations keep admin button workflows alive."""
    PREFIX = "kryzo:education:session:"

    def get(self, key, default=None):
        key = int(key)
        try:
            result = _kv_command(["GET", self.PREFIX + str(key)])
            ...
            print("KRYZO session load error:", repr(exc))
    ...
    def __setitem__(self, key, value):
        ...
        dict.__setitem__(self, key, value)
        try:
            _kv_command(["SET", self.PREFIX + str(key), json.dumps(value, ensure_ascii=False)])
        except Exception as exc:
    ...
    def pop(self, key, default=None):
        ...
        try:
            _kv_command(["DEL", self.PREFIX + str(key)])
        except Exception as exc:
            print("KRYZO session delete error:", repr(exc))
        return value
...
def ensure_webhook():
    """Keep Telegram webhook configured for both messages and button callbacks."""
    try:
        result = tg("setWebhook", {
            "url": f"{PUBLIC_URL}/api",
            "allowed_updates": json.dumps(["message", "callback_query"]),
        })
        print("KRYZO webhook ensured:", result.get("ok"))
    except Exception as exc:
        # Never block webhook processing if Telegram setup temporarily fails.
...
def handle_admin_command(message):
    ...
    if state and state.get("step") == "videos":
        ...
        if text:
            send(chat_id, "📎 Please send/forward any supported file or media, or tap <b>Generate Link</b>.")
            return True
    ...
    if state and state.get("step") == "edit_add":
        ...
        send(chat_id, "📎 Send/forward any supported file or media.")
        return True
