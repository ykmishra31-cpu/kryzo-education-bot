...
def media_from_message(message):
    """Return a generic Telegram message reference for any supported message.
    Storing chat_id + message_id lets the bot copy the original message later,
    so videos, documents, audio, voice, photos, animations, stickers, etc. work.
    """
    supported = (
        "video", "document", "audio", "voice", "photo",
        "animation", "sticker", "video_note", "contact", "location",
    )
    if any(message.get(k) for k in supported) or message.get("text") or message.get("caption"):
        return {
            "source_chat_id": message.get("chat", {}).get("id"),
            "source_message_id": message.get("message_id"),
            "title": (message.get("caption") or message.get("text") or "").strip(),
        }
    return None


def copy_message(chat_id, item):
    return tg("copyMessage", {
        "chat_id": str(chat_id),
        "from_chat_id": str(item["source_chat_id"]),
        "message_id": str(item["source_message_id"]),
    })


def send_video(chat_id, item):
    return copy_message(chat_id, item)
...
def handle_admin_command(message):
    ...
    if text == "/broadcast":
        ...
        send(chat_id, "📢 <b>Broadcast</b>\n\nSend the message/file/media you want to send to all registered users.\n\nSend <code>/cancel</code> to cancel.")
        return True
    ...
    if state and state.get("step") == "broadcast":
        if text == "/cancel":
            SESSIONS[user_id] = {"step": "idle"}
            send(chat_id, "❌ Broadcast cancelled.", admin_menu())
            return True
        ...
        for target_id in list(USERS):
            try:
                ...
                failed += 1
                print("KRYZO broadcast failed:", target_id, repr(exc))

        SESSIONS[user_id] = {"step": "idle"}
        send(chat_id, f"📢 <b>Broadcast complete</b>\n\n✅ Sent: <b>{sent}</b>\n❌ Failed: <b>{failed}</b>", admin_menu())
        return True
    ...
    if state and state.get("step") == "chapter_name":
        ...
        send(
            ...
            "Optional: put the lecture name/description in the caption.\n\n"
    ...
    if state and state.get("step") == "videos":
        ...
        if item:
            title = item["title"] or f"File {len(chapter['videos']) + 1}"
            item["title"] = title
            chapter["videos"].append(item)
            save_chapters()
            ...
            send(
                ...
                f"{esc(title)}\n\n"
                "Send another file/media, tap <b>Generate Link</b>, or send <code>/generate</code>.",
            )
            return True

        if text:
            send(chat_id, "📎 Please send/forward a file or media, or tap <b>Generate Link</b>.")
            return True
    ...
    if state and state.get("step") == "edit_add":
        ...
        if item:
            ...
            send(
                ...
                "Send another file/media or tap Done.",
                {
                    "inline_keyboard": [
        ...
        send(chat_id, "📎 Send/forward any file or media.")
        return True
...
def handle_callback(call):
    ...
    try:
        ...
        if data == "admin:users" and is_admin(user_id):
            send(chat_id, f"👥 <b>Registered Users</b>\n\nTotal: <b>{len(USERS)}</b>", admin_menu())
            callback(callback_id)
            return {"ok": True}
        ...
        if data.startswith("edit:") and is_admin(user_id):
            ...
            if action == "add":
                ...
                send(
                    ...
                    "Send or forward any file/media one by one (video, PDF, document, audio, photo, etc.).",
                    {
                        "inline_keyboard": [
