...
def telegram_joined(user_id):
    """Return True only when Telegram says the user is a channel member.

    The bot must be an administrator of TELEGRAM_CHANNEL_ID for this check.
    """
    if not TELEGRAM_CHANNEL_ID or "YOUR_TELEGRAM_CHANNEL" in TELEGRAM_CHANNEL_ID:
        print("KRYZO force-join error: TELEGRAM_CHANNEL_ID is not configured")
        return False
    ...
    try:
        ...
        member = (result or {}).get("result") or {}
