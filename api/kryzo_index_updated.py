...
BOT_TOKEN = "YOUR_BOT_TOKEN_HERE".strip()
...
def handle_admin_command(message):
    ...
    if text.startswith("/delete"):
        ...
        send(
            ...
            "This removes it from the current server memory.",
...
def handle_message(message):
    ...
    try:
        ...
        if text.startswith("/search"):
            query = text[len("/search"):].strip().lower()
            if not query:
                send(
                    chat_id,
                    "🔎 <b>Chapter Search</b>\n\n"
                    "Use <code>/search chapter name</code> to find a chapter."
                )
                return {"ok": True}

            matches = [
                c for c in CHAPTERS.values()
                if query in c["name"].lower() or query in c["id"].lower()
            ]

            if not matches:
                send(chat_id, "❌ No matching chapters found.", home_menu())
                return {"ok": True}

            for chapter in matches[:20]:
                send(
                    chat_id,
                    f"📚 <b>{esc(chapter['name'])}</b>\n"
                    f"🎥 Videos: <b>{len(chapter['videos'])}</b>",
                    {"inline_keyboard": [[
                        {"text": "▶️ Open Chapter", "url": make_link(chapter["id"])}
                    ]]},
                )
            return {"ok": True}
    ...
    try:
        ...
        if data.startswith("delete:ask:") and is_admin(user_id):
            ...
            send(
                chat_id,
                f"⚠️ <b>Delete this chapter?</b>\n\n{chapter_summary(chapter)}\n\n"
                "This removes the chapter from the current server memory.",
                delete_confirm_menu(chapter_id),
            )
            callback(callback_id)
            return {"ok": True}
